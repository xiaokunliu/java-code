...
import com.xiaokunliu.interview.j2ee.jpa.Cat;
import com.xiaokunliu.interview.j2ee.jpa.Name;
import com.xiaokunliu.interview.j2ee.jpa.Season;
...
public class JPAUsers {
    ...
    // 双向 1 一 对 N 的复合关联
    /**
     * mappedBy表明不受控制关联关系，不能使用JoinTable以及JoinColumn
     * OrderBy -- OneToMany/ManyToMany中使用
     */
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, fetch = FetchType.LAZY, targetEntity = JPAAddress.class)
    @OrderBy("detail DESC") // 进行排序返回
    private Set<JPAAddress> addresses = new HashSet<>();

    // 使用Map记录关联实体
    @OneToMany(mappedBy = "user", cascade = CascadeType.ALL, fetch = FetchType.LAZY, targetEntity = JPAAddress.class)
    @MapKey(name = "pk")
    private Map<Long, JPAAddress> addressMap = new HashMap<>();
