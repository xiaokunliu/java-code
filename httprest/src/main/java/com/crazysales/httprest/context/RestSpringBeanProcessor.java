...
public class RestSpringBeanProcessor extends SpringBeanProcessor {
	...
	protected class ResteasyBeanPostProcessor implements BeanPostProcessor {
		 ...
		 * registration
		 * <p/>
		 * <li>either singleton or request/prototype RESTeasy injection... but
		 * not registration. The RESTEasy registration happens in the
	...
	public RestSpringBeanProcessor(ResteasyDeployment deployment) {
		this(deployment.getDispatcher(), deployment.getRegistry(), deployment.getProviderFactory());
	}

	public RestSpringBeanProcessor(Dispatcher dispatcher) {
		this(dispatcher, dispatcher.getRegistry(), dispatcher.getProviderFactory());
	}

	public RestSpringBeanProcessor(Dispatcher dispatcher, Registry registry, ResteasyProviderFactory providerFactory) {
		this.setRegistry(registry);
		this.setProviderFactory(providerFactory);
		this.setDispatcher(dispatcher);
	}

	public RestSpringBeanProcessor() {
	}
	...
	/**
	 * <p>
	 * This method keeps track of @Provider and resources for future use. It
	 * also registers the RESTEasy Registry, ProviderFactry, and Dispatcher for @Autowire
	 * injection.
	 * </p>
	 * <p/>
	 * <p>
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		...
		for (String name : beanFactory.getBeanDefinitionNames()) {
			...
			if ((beanDef.getBeanClassName() != null || beanDef.getFactoryBeanName() != null) && !beanDef.isAbstract())
	 ...
	 */
	protected Class<?> processBean(final ConfigurableListableBeanFactory beanFactory, List<String> dependsOnProviders, String name, BeanDefinition beanDef) {
    ...
    private static Class<?> getBeanClass(String name, BeanDefinition beanDef,
        ...
        if (beanDef instanceof RootBeanDefinition) {
            ...
            try {
                ...
                int skip = 0; // need to do something
        ...
        if (factoryMethodName != null) {
            ...
            } else if (beanDef instanceof FactoryBean) {
                try {
                    factoryClassName = ((FactoryBean) beanDef).getObject().getClass().getName();
                } catch (Exception ex) {
                    throw new IllegalStateException("could not build factory object for " + name, ex);
                }
            } else {
            ...
            // ------ section added to properly handle Activiti factory beans
            Class beanFactoryClass = getBeanClass(factoryClassName);
            if (FactoryBean.class.isAssignableFrom(beanFactoryClass)) {
                try {
                    Method method = beanFactoryClass.getMethod("getObject");
                    if (null != method) {
                        factoryClassName = method.getReturnType().getName();
                    }
                } catch (Exception ex) {
                    throw new IllegalStateException("could not find the type for bean named " + name +
                            ", cannot get bean on factory", ex);
                }
            }
            // ------ end of section
	...
	public void onApplicationEvent(ApplicationEvent event) {
		...
		// The following code would re-process the bean factory, in case the
		// configuration changed.
		// However, it needs work.
		// if (event.getSource() instanceof XmlWebApplicationContext)
		// {
		// ConfigurableListableBeanFactory beanFactory =
		// ((XmlWebApplicationContext) event.getSource()).getBeanFactory();
